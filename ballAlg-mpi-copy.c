...
node_t *build_tree_parallel_mpi(long start, long end, int process, int max_processes, int threads) {  
    ...
    MPI_Request requests[max_processes - 1];
    ...
    if (first == 0){
        ...
        for (int i = process + 1, d = 1; i < max_processes; i++, d++) {
            MPI_Isend(for_args, 4, MPI_LONG, i, FOR_TAG, WORLD, &requests[d - 1]);
        }
        //MPI_Waitall(max_processes - (process + 1), requests, MPI_STATUS_IGNORE);
        first = 1;
    }
    // Second for onwards
    else {
        ...
        for (int i = process + 1, d = 1; i < max_processes; i++, d++) {
            ...
            MPI_Isend(points_indexes[i], interval, MPI_LONG, i, POINT_TAG, WORLD, &requests[d - 1]);
    ...
    if (process + 1 < max_processes) {
        ...
        MPI_Isend(args, 2, MPI_LONG, new_max_processes, ARGS_TAG, WORLD, &requests[0]);
        ...
        MPI_Isend(points_index, size, MPI_LONG, new_max_processes, POINT_TAG, WORLD, &requests[0]);
...
void wait_mpi(int me, int begin, int end, int threads) {
    ...
    if (first == 0){
        ...
        MPI_Irecv(for_indexes, interval, MPI_LONG, MPI_ANY_SOURCE, POINT_TAG, WORLD, &request);
    ...
    MPI_Irecv(rec_index, args[0], MPI_LONG, MPI_ANY_SOURCE, POINT_TAG, WORLD, &request2);
